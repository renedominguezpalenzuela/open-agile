export const blog_details = [
  {
    id: "1",
    title: "BUCHEMPFEHLUNGEN",
    titulo2: "“EINE OPEN AGILE AUSWAHL”",
    image: "blogs/blog_buchempfehlungen.png",
    autor: "",
    fecha: "",
    autor: "Andreas Eilers",
    fecha: "14.01.2022",

    content: [
      {
        tipo: "parrafo",
        text: [
          "Wir haben alle Meinungen und Ansichten, die ihren Ursprung in unserer individuellen Biographie finden. Unter anderem werden unsere Ansichten durch die Auswahl von Büchern geformt und gleichermaßen bestätigt. Meinungen sind nicht per se richtig oder falsch. Wir können Meinungen und Ansichten austauschen, diskutieren und oftmals auch anpassen und korrigieren. Viele unserer Ansichten und Ideen stammen aus folgenden Buchempfehlungen. Auch wenn wir die Lektüre mit Faszination gelesen haben, stimmen wir mit den Inhalten der Schriften keinesfalls komplett überein. Es sollte stets kritisch hinterfragt werden, um letztendlich überhaupt eine eigene Meinung haben zu können.",
        ],
      },
      {
        tipo: "puntos",
        titulo: "",
        text: [
          "Reinventing Organizations - Laloux, Frederic",
          "Selbstorganisierte Unternehmen - Kaltenecker, Siegfried",
          "Selbstorganisation braucht Führung - Gloger, Boris",
          "Netzwerk schlägt Hierarchie - Brandes-Visbeck, Christiane & Gensinger",
           "Netzwerke – Bensmann, Dieter",
          "Erfolg ohne Chef – Plüger, Gernot",
          "Hierarchielos führen – Moser, Michaela",
          "Hierarchie. Das Ende eines Erfolgsrezepts – Frei, Felix",
          "Jenseits der Hierarchie – Lehner, M. Johannes, Ötsch, O. Walter",
          "Das kollegial geführte Unternehmen – Oestereich, Bernd & Schröder",
          "Agile Organisationen – Häusling, André",
          "Agiles Coaching – Andresen, Judith",
          "Die Kraft von Scrum – Wolf, Henning",
          "Scrum: Verstehen und erfolgreich umsetzen – Rock, Stefan",
          "Kanban in der Praxis – Leopold, Klaus",
          "Lean Startup – Ries, Eric",
          "Agile Skalierung – Nowotny, Valentin",
          "Workhacks – Schültken, Lydia",
          "Agil Moderieren – Stach, Michaela",
          "Mit Achtsamkeit in Führung – Rothes, Paul & Rosmann, Nadja",
          "Lieblosigkeit macht krank – Hüther, Gerald",
          "Würde – Hüther, Gerald",
          "Der Bienen Hirte – Van Solingen, Rini",
          "Die Kraft der Mediation – Sedlmeier, Peter",
          "Gewaltfreie Kommunikation – Rosenberg Marshall",
          "Trainingsbuch GFK – Holler, Ingrid",
          "Mut zur Improvisation – Cordes, Charlotte",
          "Impro-Theater – List, Volker",
        ],
      },
      
    ],
    link_anterior: "/blog/3",
    link_anterior_nombre: "BUCHVERÖFFENTLICHUNG",
    link_anterior_nombre2: "“CRASHKURS SELBSTORGANISATION IN AGILEN TEAMS”",

    link_siguiente: "/blog/2",
    link_siguiente_nombre: "WORKHACKS",
    link_siguiente_nombre2:
      "“WORKHACKS SIND EINE REVOLUTION. EINE LEISE REVOLUTION.”",
  },

  {
    id: "2",
    title: "WORKHACKS",
    titulo2: "“WORKHACKS SIND EINE REVOLUTION. EINE LEISE REVOLUTION.”",
    image: "blogs/blog_workhacks.png",
    autor: "",
    fecha: "",
    content: [
      {
        tipo: "parrafo",
        text: [
          "Workhacks haben ihren Ursprung bei den sogenannten Lifehacks, die durch die Zusammenführung der Worte „life” und „hack” im Jahr 2004 bekannt wurden. Lifehacks sind Lebenskniffe, die sich auf Unwägbarkeiten, Strategien oder Tätigkeiten des Lebens beziehen.[1] Sie dienen dazu, ein Problem zu lösen, das Ziel auf eine ungewöhnliche Weise zu erreichen oder die Effektivität zu erhöhen, immer mit dem Ziel den Alltag zu erleichtern. 2005 wurde das Wort „Lifehack” schließlich zum zweitnützlichsten Wort des Jahres gekürt.",
          "Lydia Schültken definiert workhacks in ihrem gleichnamigen Buch wie folgt: „workhacks sind eine Revolution. Eine leise Revolution. Als minimalinvasive Eingriffe in die Zusammenarbeit und die Organisation von Teams entfalten sie mehr Wirkung als jedes verordnete Change-Programm. Agil, selbstbestimmt und nachhaltig.“[2] Meiner Ansicht nach sind workhacks dafür gut, Teams dabei zu unterstützen sich selbst zu organisieren, wenn durch den Wegfall von formeller, hierarchischer Führung ein Vakuum entsteht, welches durch die Entfaltung der neu erworbenen persönlichen und fachlichen Kompetenz wieder aufgefüllt wird. workhacks sind im Grunde genommen alle Tools, die nicht zu der üblichen Stellenbeschreibung der Mitarbeitenden gehören, welche die Arbeit in selbstorganisierten Teams allerdings in irgendeiner Form verbessert oder einfacher macht und darüber hinaus zeitsparend eingesetzt werden können.",
          "Bei der Verbreitung und Anwendung der workhacks geht es in meinen Augen um Kooperation und nicht um Konkurrenz zueinander. Je mehr Menschen diese Tools in ihren Teams einsetzen, desto schneller kommen wir ans Ziel, eine stimmigere und würdevollere Arbeitswelt zu kreieren.",
          "Auch Schültken schreibt in ihrem Buch, dass workhacks unabhängig vom Erfinder eingesetzt werden können, sozusagen als „open space code”. Darüber hinaus beruhen workhacks auf vier Prinzipien:",
        ],
      },
      {
        tipo: "numeros",
        text: [
          "Workhacks verändern Arbeitsabläufe, brechen eingefahrene Routinen auf, ohne dabei irreparable Schäden zu hinterlassen. Sie ermutigen Teams zu mehr Selbstverantwortung, Effektivität, Innovation und manchmal nur zu besserer Zusammenarbeit.",
          "Workhacks werden nicht von oben angeordnet. Jedes Team und jede Abteilung kann die Veränderung für sich entscheiden – statt Befehlsverweigerung zu provozieren, machen workhacks Lust auf Veränderung. ",
          "Workhacks werden als Experimente eingeführt und konsequent abgeschafft, wenn sie nicht hilfreich sind. Sie werden aber auch konsequent weitergeführt, wenn sie hilfreich sind. ",
          "Workhacks können schnell eingeführt werden – ohne langwierige Vorstandsentscheide, ohne Soll-ist-Analysen, ohne systemische Strategieberatung. Sie sind praktisch und intelligent zugleich. ",
        ],
      },
      {
        tipo: "numero_corchetes",
        text: [
          "[1] https://de.wikipedia.org/wiki/Lifehack (abgerufen am 03.06.2020).",
          "[2] Lydia Schültken: workhacks. Sechs Angriffe auf eingefahrene Arbeitsabläufe. Haufe Verlag, 2017, S. 7-10",
        ],
      },
    ],
    link_anterior: "/blog/1",
    link_anterior_nombre: "BUCHEMPFEHLUNGEN",
    link_anterior_nombre2: "“EINE OPEN AGILE AUSWAHL”",

    link_siguiente: "/blog/3",
    link_siguiente_nombre: "BUCHVERÖFFENTLICHUNG",
    link_siguiente_nombre2: "“CRASHKURS SELBSTORGANISATION IN AGILEN TEAMS”",
  },

  {
    id: "3",
    title: "BUCHVERÖFFENTLICHUNG",
    titulo2: "“CRASHKURS SELBSTORGANISATION IN AGILEN TEAMS”",
    image: "blogs/blog_buchveröffentlichung.png",
    autor: "Andreas Eilers",
    fecha: "14.01.2022",

    content: [
      {
        tipo: "parrafo",
        text: [
            "Viele Beschäftigte machen Dienst nach Vorschrift oder haben bereits innerlich gekündigt. Das liegt allerdings nicht an ihrer Tätigkeit, für die man sie aufgrund ihrer Stärken einmal eingestellt hat, sondern an den veralteten und starren Strukturen, die seit dem Industriezeitalter niemand mehr angefasst hat.",
          "Mit veralteten, starren Strukturen meine ich die zahlreichen Auslaufmodelle, wie z.B. starre Hierarchien, in denen Vorgesetzte keine guten Argumente benötigen, sondern einfach qua Amt die Marschrichtung vorgeben. Damit meine ich aber auch antike Arbeitszeitmodelle oder unsinnige Vorgaben „wo“ und „wie“ Mitarbeitende zu arbeiten haben",
          "Ich bin der Meinung, dass Menschen grundsätzlich selbst am besten Wissen wie, wo und wann sie am produktivsten sind.",
          "Was müssen wir also tun, um im Zeitalter der Digitalisierung alle Beschäftigten gleichermaßen für innovative Ideen gewinnen zu können?",
          "Die Lösung ist eine flexiblere und dynamischere Zusammenarbeit. Diese ergibt sich durch mehr Handlungsspielraum für alle Mitarbeitenden und eine größere Eigenständigkeit. Kurz gesagt: durch Selbstorganisation.",
          "In meinem Buch beschreibe ich neben den zahlreichen Gründen für die Selbstorganisation einen konkreten Fahrplan zu einer Organisation, die Euch wieder Mitdenken lässt. Dazu beschreibe ich die Architektur selbstorganisierter Teams, wie diese z.B. selbstständig Entscheidungen treffen, welches Mindset dafür wichtig ist, welche neuen Rollen gebraucht werden oder wie sie Konflikte wertschätzend und auf Augenhöhe lösen können.",
          "Darüber hinaus stelle ich 20 bereits bewährte Workhacks vor. Das sind einfache agile Methoden, die Teams schnell und flexibel einsetzen, um besser, effizienter oder einfach menschlicher zusammenzuarbeiten.  "
        ],
      },
      
    ],
    link_anterior: "/blog/2",
    link_anterior_nombre: "WORKHACKS",
    link_anterior_nombre2:
      "“WORKHACKS SIND EINE REVOLUTION. EINE LEISE REVOLUTION.”",

    link_siguiente: "/blog/1",
    link_siguiente_nombre: "BUCHEMPFEHLUNGEN",
    link_siguiente_nombre2: "“EINE OPEN AGILE AUSWAHL”",
  },

  //  {
  //     "id":"4",
  //     "title" : "BUCHEMPFEHLUNGEN",
  //     "titulo2" : '“EINE OPEN AGILE AUSWAHL”',
  //     "image":"blogs/blog03.jpeg",
  // } ,
  //   {
  //     "id":"5",
  //     "title" : "BUCHVERÖFFENTLICHUNG",
  //     "titulo2" : '“CRASHKURS SELBSTORGANISATION IN AGILEN TEAMS”',
  //     "image":"blogs/blog01.png",
  // },
];
